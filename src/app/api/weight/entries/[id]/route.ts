...
import { getEntryById, hardDeleteEntry, patchEntry, softDeleteEntry } from "@/lib/weight/repo";
import { weightPatchSchema } from "@/lib/weight/schema";
...
export async function PATCH(request: NextRequest, { params }: Ctx) {
  const auth = requireBearer(request);
  if (!auth.ok) return auth.response;
  const parsed = await parseJson(request, weightPatchSchema);
...
export async function DELETE(request: NextRequest, { params }: Ctx) {
  ...
  return done ? noContent() : notFound("Weight entry not found");
}
